...
public class DataLoader implements CommandLineRunner {
    ...
    private final PlataformaBeta       grafoService;
    ...
    private void cargarDesdeArchivos() {
        ...
        for (Cliente c : persistencia.cargarClientes()) {
            try {
                ...
                grafoService.agregarNodoGrafo(c.getId());
            } catch (Exception ignored) {}
        }

        // 3. Inmuebles — requiere vincular asesor ya cargado
        for (Inmueble i : persistencia.cargarInmuebles()) {
            try {
                // Resolver asesor desde repositorio
                if (i.getAsesor() != null) {
                ...
                grafoService.agregarNodoGrafo(i.getCodigo());
            } catch (Exception ignored) {}
        }

        // 4. Visitas — vincular cliente, inmueble, asesor
        for (Visita v : persistencia.cargarVisitas()) {
            try {
                ...
                // Reconstruir arista en grafo
                if (v.getCliente() != null && v.getInmueble() != null) {
                    grafoService.agregarAristaGrafo(v.getCliente().getId(), v.getInmueble().getCodigo());
                }
            } catch (Exception ignored) {}
        }
    ...
    private void cargarVisitas() {
        Object[][] visitas = {
                {"VIS-001","CLI-001","INM-001",LocalDate.now().minusDays(10),LocalTime.of(10,0),"ASR-001","Muy interesado"},
                {"VIS-002","CLI-002","INM-003",LocalDate.now().minusDays(8),LocalTime.of(11,30),"ASR-002","Evaluando sector"},
                {"VIS-003","CLI-003","INM-002",LocalDate.now().minusDays(7),LocalTime.of(15,0),"ASR-001","Cumple expectativas"},
                {"VIS-004","CLI-004","INM-016",LocalDate.now().minusDays(6),LocalTime.of(9,0),"ASR-001","Interesada en precio"},
                {"VIS-005","CLI-005","INM-014",LocalDate.now().minusDays(5),LocalTime.of(14,0),"ASR-004","Comparando inmuebles"},
                {"VIS-006","CLI-006","INM-005",LocalDate.now().minusDays(5),LocalTime.of(16,0),"ASR-003","Requiere ampliación"},
                {"VIS-007","CLI-007","INM-012",LocalDate.now().minusDays(4),LocalTime.of(10,30),"ASR-002","Flujo peatonal alto"},
                {"VIS-008","CLI-008","INM-006",LocalDate.now().minusDays(3),LocalTime.of(11,0),"ASR-003","Buen estado general"},
                {"VIS-009","CLI-001","INM-004",LocalDate.now().minusDays(3),LocalTime.of(15,30),"ASR-002","Segunda visita"},
                {"VIS-010","CLI-003","INM-008",LocalDate.now().minusDays(2),LocalTime.of(9,30),"ASR-004","Evaluando barrio"},
        };
